...
class _WebViewExampleState extends State<WebViewExample> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Conning Tower'),
      ...
      body: WebView(
        initialUrl: 'https://www.dmm.com/netgame/social/-/gadgets/=/app_id=854854/',
...
class SampleMenu extends StatelessWidget {
      ...
      builder:
        ...
        return PopupMenuButton<MenuOptions>(
          ...
          onSelected: (MenuOptions value) {
            switch (value) {
              case MenuOptions.openBlank:
                _onOpenBlank(controller.data!, context);
                break;
          ...
          itemBuilder: (BuildContext context) => <PopupMenuItem<MenuOptions>>[
            PopupMenuItem<MenuOptions>(
              value: MenuOptions.openBlank,
              enabled: controller.hasData,
              child: const Text('Open Blank'),
            ),
            PopupMenuItem<MenuOptions>(
  ...
  Future<void> _onOpenBlank(
      WebViewController controller, BuildContext context) async {
    // String javaScriptString = """((\$,_)=>{const html=\$.documentElement,gf=\$.getElementById('htmlWrap'),gs=gf.style,gw=gf.offsetWidth,gh=gw*.6;let vp=\$.querySelector('meta[name=viewport]'),t=0;vp||(vp=\$.createElement('meta'),vp.name='viewport',\$.querySelector('head').appendChild(vp));vp.content='width='+gw;'orientation'in _&&html.webkitRequestFullscreen&&html.webkitRequestFullscreen();html.style.overflow='hidden';\$.body.style.cssText='min-width:0;overflow:hidden;margin:0';gs.position='fixed';gs.marginRight='auto';gs.marginLeft='auto';gs.top='-5px';gs.right='0';gs.zIndex='100';gs.transformOrigin='50%25 16px';if(!_.kancolleFit){const k=()=>{const w=html.clientWidth,h=_.innerHeight;w/h<1/.6?gs.transform=%27scale(%27+w/gw+%27)%27:gs.transform=%27scale(%27+h/gh+%27)%27;w<gw?gs.left=%27-%27+(gw-w)/2+%27px%27:gs.left=%270%27};_.addEventListener(%27resize%27,()=>{clearTimeout(t);t=setTimeout(k,10)});_.kancolleFit=k}kancolleFit()})(document,window)""";
    // await controller.runJavascript('''window.open("https://www.google.com/");''');
    await controller.runJavascript("""Toaster.postMessage("KanColle open");""");
